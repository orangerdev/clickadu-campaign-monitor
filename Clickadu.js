class ClickAduCampaigns {
  ...
  clearCampaigns() {
    const startCell = SHEET_CAMPAIGN.getRange("A2:G");
    
    const startRow = startCell.getRow();
    const startColumn = startCell.getColumn();
    
    const lastRow = SHEET_CAMPAIGN.getLastRow();
    const lastColumn = SHEET_CAMPAIGN.getLastColumn()+1;
    ...
    const range = SHEET_CAMPAIGN.getRange(startRow, startColumn, lastRow - startRow + 1, lastColumn - startColumn + 1);
  ...
  getCampaigns() {
      ...
      const strMinDate = minDate.getFullYear() + '-' + addLeadingZero(minDate.getMonth() + 1) + '-' + addLeadingZero(minDate.getDate())
      const strMaxDate = maxDate.getFullYear() + '-' + addLeadingZero(maxDate.getMonth() + 1) + '-' + addLeadingZero(maxDate.getDate())
      ...
      for(let thePage = 1; thePage <= TOTAL_PAGES; thePage++) {
        ...
        response.result.items.forEach((campaign) => {
          ...
          if(
            ...
            const nextRow = SHEET_CAMPAIGN.getLastRow() + 1

            SHEET_CAMPAIGN.getRange(nextRow, 1, 1, 9).setValues([
              [
                campaign.id,
                campaign.name,
  ...
  stopCampaigns () {
      ...
      const theValues = SHEET_STOPCAMPAIGN.getRange("A1:A" + theLastRow).getValues();

      if(theValues.length == 0) 
        return;

      if(theValues[0].length == 0)
        return;

      if(theValues[0][0] == "#N/A")
        return;
      ...
      if(response.result === "success") {
        ...
        writeLog(`Cant stop campaigns : ${campaigns.join(", ")} | Reason: ${response.error.message}`)
      }
  }
  ...
  rerunCampaigns () {
    ...
    const theValues = SHEET_RERUNCAMPAIGN.getRange("A1:A" + theLastRow).getValues();
    ...
    if(response.result === "success") {
      ...
      writeLog(`Cant start campaigns : ${campaigns.join(", ")} | Reason: ${response.error.message}`)
    }
  }
}
